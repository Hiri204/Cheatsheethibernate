...
public class UserServiceImpl implements UserService {
    ...
    public User findByUsernameOrEmail(String loginId) {
        ...
        // Try username first
        User user = userRepository.findByUsername(loginId.trim());
        if (user != null) {
            return user;
        }
        // Then try email
        return userRepository.findByEmail(loginId.trim());
    }
    ...
    public void suspendUser(Integer userId, String reason) {
        ...
        // 🎯 NOTIFICATION: Notify user about suspension
        notificationService.notifyBan(user, reason);
    }
    ...
    public void banUser(Integer userId, String reason, String currentStatus, Integer adminId) {
        ...
        // 🎯 NOTIFICATION: Notify user about ban
        notificationService.notifyBan(user, reason);
    }
    ...
    public void banUserWithDuration(Integer userId, String reason, Integer durationDays, Integer adminId) {
        ...
        // 🎯 NOTIFICATION: Notify user about ban with duration
        String banMessage = reason + " (Duration: " + (durationDays != null ? durationDays + " days" : "Permanent") + ")";
        notificationService.notifyBan(user, banMessage);
    }
    ...
    public void followUser(Integer followerId, Integer followingId) {
        ...
        // 🎯 NOTIFICATION: Notify the user that someone followed them
        User follower = getById(followerId);
        User following = getById(followingId);

        if (following != null && follower != null && !followerId.equals(followingId)) {
    ...
    public void sendNotificationToAllUsers(String subject, String message, Integer adminId) {
        ...
        List<User> users = getAllUsers();
        for (User user : users) {
            if (user != null && !user.getUserId().equals(adminId)) {
                notificationService.createNotification(user, 
                    cheatsheethibernate.entity.Notification.NotificationType.ANNOUNCEMENT,
                    message, "announcement", null, adminId);
            }
        }
    }
}