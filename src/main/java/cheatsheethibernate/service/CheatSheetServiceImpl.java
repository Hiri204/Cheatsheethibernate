...
/**
 * Service implementation for CheatSheet management.
 * Provides business logic for cheat sheet CRUD operations, user authentication,
 * review and comment management, and ban management.
 */
@Service
@Transactional
public class CheatSheetServiceImpl implements CheatSheetService {
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CheatSheet> getCheatSheetsByTagName(String tagName) {
        return cheatSheetRepository.findCheatSheetsByTagName(tagName);
    }
    ...
    public List<CheatSheet> getCheatSheetsByTagAndStatus(String tagName, CheatSheet.Status status) {
        ...
        return sheets.stream()
                .filter(sheet -> sheet.getStatus() == status)
    ...
    public void addReview(Long cheatSheetId, User user, Integer rating, String text) {
        ...
        // 🎯 NOTIFICATION: Notify sheet owner about review
        if (cheatSheet.getUser() != null && !cheatSheet.getUser().getUserId().equals(user.getUserId())) {
            notificationService.notifyReview(cheatSheet.getUser(), user, "cheat sheet", cheatSheetId);
        }
    }
    ...
    public void addOrUpdateReview(Long cheatSheetId, User user, Integer rating, String text) {
        ...
        if (existingReview != null) {
            ...
            // 🎯 NOTIFICATION: Notify sheet owner about new review
            if (cheatSheet.getUser() != null && !cheatSheet.getUser().getUserId().equals(user.getUserId())) {
                notificationService.notifyReview(cheatSheet.getUser(), user, "cheat sheet", cheatSheetId);
            }
        }
    ...
    public void addComment(Long cheatSheetId, User user, String content, Integer parentId) {
        ...
        User commentTargetUser = cheatSheet.getUser();
        ...
        if (parentId != null) {
            ...
            if (parent != null) {
                ...
                isReply = true;
                commentTargetUser = parent.getUser();
            }
        }
        ...
        // 🎯 NOTIFICATION: Notify based on comment type
        if (isReply) {
            if (commentTargetUser != null && !commentTargetUser.getUserId().equals(user.getUserId())) {
                notificationService.notifyReply(commentTargetUser, user, "cheat sheet", cheatSheetId);
            }
        } else {
            if (commentTargetUser != null && !commentTargetUser.getUserId().equals(user.getUserId())) {
                notificationService.notifyComment(commentTargetUser, user, "cheat sheet", cheatSheetId);
            }
        }
    }
    ...
    public void banSheet(Long sheetId, String reason, LocalDateTime expiresAt, Integer adminId) {
        ...
        // 🎯 NOTIFICATION: Notify sheet owner about ban
        if (sheet.getUser() != null) {
            notificationService.notifyBan(sheet.getUser(), reason);
        }
    }
    ...
    public void incrementBookmarkCount(Long sheetId) {
        // Using SessionFactory for direct HQL update (from version 2)
        Session session = sessionFactory.getCurrentSession();
        Query<?> query = session.createQuery(
            "UPDATE CheatSheet c SET c.bookmarkCount = c.bookmarkCount + 1 WHERE c.id = :id"
        );
        query.setParameter("id", sheetId);
        query.executeUpdate();
    }
    ...
    public void decrementBookmarkCount(Long sheetId) {
        ...
        Session session = sessionFactory.getCurrentSession();
        Query<?> query = session.createQuery(
            "UPDATE CheatSheet c SET c.bookmarkCount = c.bookmarkCount - 1 WHERE c.id = :id AND c.bookmarkCount > 0"
        );
        query.setParameter("id", sheetId);
        query.executeUpdate();
    }
    ...
    public void publishSheet(Long sheetId) {
        ...
        // 🎯 NOTIFICATION: Notify followers when a new sheet is published
        CheatSheet sheet = cheatSheetRepository.findById(sheetId);
        if (sheet != null && sheet.getUser() != null) {
            System.out.println("📢 New sheet published: " + sheet.getTitle() + " by " + sheet.getUser().getUsername());
