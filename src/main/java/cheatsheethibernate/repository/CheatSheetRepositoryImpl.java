...
public class CheatSheetRepositoryImpl implements CheatSheetRepository {
	...
	public List<CheatSheet> findByTagId(Integer tagId) {
		...
		@Override
		public List<CheatSheet> findCheatSheetsByTagName(String tagName) {
		    String lowerTagName = (tagName != null) ? tagName.toLowerCase() : "";
		    
		    // 🎯 FIX: Tag နှင့် ချိတ်ဆက်နေပြီး published ဖြစ်သော CheatSheet များကိုသာ ဆွဲထုတ်ခြင်း
		    String hql = "SELECT c FROM CheatSheet c JOIN c.tags t " +
		                 "WHERE lower(t.name) = :tagName AND c.status = 'published' " +
		                 "ORDER BY c.createdAt DESC";
		                 
		    return sessionFactory.getCurrentSession()
		            .createQuery(hql, CheatSheet.class)
		            .setParameter("tagName", lowerTagName)
		            .getResultList();
		}
