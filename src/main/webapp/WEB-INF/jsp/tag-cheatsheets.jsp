...
<title>Tag: ${selectedTag} · DevSheets</title>
...
<body>
	...
	<div class="main-content">
		...
		<div
			...
			<div>
				<h3 class="page-title">
					<i class="fa-solid fa-tag text-primary me-2"></i>Tag: #${selectedTag}
				</h3>
					...
					<div class="sheet-card ${sheet.status eq 'banned' ? 'banned' : ''}">
						...
						<c:choose>
							<c:when test="${not empty sheet.fileUrl}">
								<img
									src="${pageContext.request.contextPath}${sheet.fileUrl}"
						...
						<div class="card-body">
							...
							<div class="tags-container">
								<c:forEach var="tag" items="${sheet.tags}">
									<a href="${pageContext.request.contextPath}/cheatsheets/tag/${tag.name}" class="text-decoration-none">
										<span class="tag-badge ${tag.name eq selectedTag ? 'active-tag' : ''}" style="cursor: pointer;"> 
							...
							<div class="meta-info mb-3">
								<span class="d-block mb-1"> 
									...
									<a href="${pageContext.request.contextPath}/user/profile/${sheet.user.userId}" class="author"> 
										${sheet.user != null ? sheet.user.username : 'Unknown'}
									</a>
								</span>
								
								<div class="p-2 bg-light rounded-3 mt-1" style="font-size: 0.7rem; color: #6c757d;">
									<div class="mb-1">
										<i class="fa-regular fa-calendar-plus me-1 text-success"></i> Created:
										<fmt:parseDate value="${sheet.createdAt}" pattern="yyyy-MM-dd'T'HH:mm:ss" var="parsedCreated" type="both" />
										<fmt:formatDate value="${parsedCreated}" pattern="dd MMM yyyy, hh:mm a" />
									</div>
									<c:if test="${not empty sheet.updatedAt && sheet.updatedAt != sheet.createdAt}">
										<div>
											<i class="fa-regular fa-calendar-check me-1 text-primary"></i> Updated:
											<fmt:parseDate value="${sheet.updatedAt}" pattern="yyyy-MM-dd'T'HH:mm:ss" var="parsedUpdated" type="both" />
											<fmt:formatDate value="${parsedUpdated}" pattern="dd MMM yyyy, hh:mm a" />
										</div>
									</c:if>
								</div>
							</div>
			...
			<c:if test="${empty sheets}">
				<div class="col-12">
					<div class="empty-state">
						...
						<h5>ဤ Tag (#${selectedTag}) အောက်တွင် မည်သည့် Cheat Sheet မျှ မရှိသေးပါ။</h5>
						<p class="text-muted small">တခြား Tag ခေါင်းစဉ်များကို ရှာဖွေကြည့်ပါဦးဗျာ။</p>
					</div>
				</div>
			</c:if>
		</div>
	...
	<script>
        ...
        function toggleLike(sheetId, element) {
            ...
            .then(function(data) {
                if (data.success) { countSpan.textContent = data.likeCount; }
            })
        ...
        function showAlert(type, message) {
            ...
            var iconClass = type === 'success' ? 'fa-circle-check text-success' : (type === 'warning' ? 'fa-circle-exclamation text-warning' : 'fa-circle-exclamation text-danger');
        ...
        function openReportModal(sheetId, sheetTitle) {
            if (typeof bootstrap === 'undefined') {
                ...
                script.onload = function() { openReportModal(sheetId, sheetTitle); };
            ...
            for (var i = 0; i < reasonBtns.length; i++) { reasonBtns[i].classList.remove('active'); }
        ...
        function submitReportAsync(event) {
            ...
            .then(function(response) { return response.json(); })
            .then(function(data) {
                var modalInstance = bootstrap.Modal.getInstance(document.getElementById('reportModal'));
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            console.log('Tag results page initialized successfully');
        });
    </script>
</body>
</html>