...
<title>Following · DevSheets</title>
	...
	<div class="main-content">
		...
		<div class="d-flex justify-content-start mb-4">
			<ul class="nav nav-pills nav-pills-custom gap-2 p-1 bg-white rounded-pill shadow-sm">
				<li class="nav-item">
					<a class="nav-link" href="${pageContext.request.contextPath}/user/followers/${user.userId}">
						<i class="fa-solid fa-users me-1"></i> Followers <span class="badge bg-light text-dark ms-1">${followerCount}</span>
					</a>
				</li>
				<li class="nav-item">
					<a class="nav-link active" href="${pageContext.request.contextPath}/user/following/${user.userId}">
				...
				<c:choose>
					<c:when test="${empty following}">
						<div class="text-center py-5 bg-white rounded-4 border border-dashed">
							<i class="fa-solid fa-user-plus text-muted mb-3" style="font-size: 3rem; opacity: 0.4;"></i>
							<h5 class="text-muted fw-semibold">Not following anyone yet</h5>
							<p class="text-muted small">When this user follows other accounts, they will appear here.</p>
						</div>
					</c:when>
					<c:otherwise>
						<c:forEach var="fUser" items="${following}">
